...
#define LE_ARRAY_TO_UINT16(u16, a)                                                                                                 \
    {                                                                                                                              \
        u16 = ((uint16_t) (*(a + 0)) << 0) + ((uint16_t) (*(a + 1)) << 8);                                                         \
    }

/** Little Endian array to uint32. */
#define LE_ARRAY_TO_UINT32(u32, a)                                                                                                 \
    {                                                                                                                              \
        u32 = ((uint32_t) (*(a + 0)) << 0) + ((uint32_t) (*(a + 1)) << 8) + ((uint32_t) (*(a + 2)) << 16) +                        \
            ((uint32_t) (*(a + 3)) << 24);                                                                                         \
    }
    ...
    if (matter_ota_db->state == MATTER_OTA_STATE_PACK_HEADER)
    {
        if (matter_ota_db->buf_offset >= PACK_HEADER_LEN1)
        {
            ...
            if (matter_ota_db->buf_offset >= pack_header_len)
            {
                ...
                memcpy((void *) matter_ota_db->p_sub_image_header, matter_ota_db->p_buf + PACK_HEADER_LEN1,
...
bool rtk_matter_ota_process_mp_header()
{
    ...
    if (matter_ota_db->state == MATTER_OTA_STATE_IMAGE_HEADER)
    {
        if (matter_ota_db->buf_offset >= MP_HEADER_SIZE)
        {
            ...
            LOG_INF("rtk_matter_ota_process_mp_header: image id 0x%04x, payload size 0x%x", matter_ota_db->cur_image_id,
            ...
            if ((matter_ota_db->active_bank == OTA_BANK0 && matter_ota_db->cur_image_index < matter_ota_db->sub_image_num / 2) ||
                (matter_ota_db->active_bank == OTA_BANK1 && matter_ota_db->cur_image_index >= matter_ota_db->sub_image_num / 2))
            {
...
bool rtk_matter_ota_process_payload()
{
    ...
    {
        if (matter_ota_db->cur_download_size + matter_ota_db->buf_offset < matter_ota_db->cur_image_payload_size)
        {
            if (matter_ota_db->is_data_valid)
            {
                if (dfu_write_data_to_flash(matter_ota_db->cur_image_id, matter_ota_db->cur_download_size, 0,
                                            matter_ota_db->buf_offset, matter_ota_db->p_buf) != 0)
                {
                    ret = 1;
        ...
        else
        {
            ...
            if (matter_ota_db->is_data_valid)
            {
                if (dfu_write_data_to_flash(matter_ota_db->cur_image_id, matter_ota_db->cur_download_size, 0, last_pkt_len,
